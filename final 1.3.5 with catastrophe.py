...
def biome(PythonShine,notPythonShinecolour,PythonShinecolour,Position_new):
    draw(PythonShine,notPythonShinecolour)   
    draw(1,PythonShinecolour)
    draw(7-PythonShine,notPythonShinecolour)
    t.home()
    goback()
    t.penup()
    t.right(90)
    t.forward(39)
    t.left(90)
    t.forward (Position_new * 70 + 27)
    t.pendown()
    t.dot(30, "green")
    
def turtle(val1,val2,question,Position_new):
    answer = askTwoValues(val1,val2,question)
    if answer == "y":
        print "[Close the turtle window to resume]"
        t.speed(0)
        goback()
        biome (PythonShine,"yellow","purple",Position_new)
        t.done()

def convert_to_binary(thing):
    a = map(int,thing)
    b = ''
...
############################### data generators ##############################################
    ...
    catastrophe = numberGenerator (0,29)  #catasptrophe
...
########################### start to loop ################################################
    ...
    for i in range (MaximumTurns):
        if catastrophesBoolean == 'y':
            if catastrophe < 8:
                del listOfList[catastrophe]
        print
        ...
        turtle('y','n','Do you want to see the turtle board? (y/n)',Position_new)
